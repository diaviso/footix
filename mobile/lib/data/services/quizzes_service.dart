...
class QuizzesService {
  ...
  Future<QuizModel> getRevisionQuiz() async {
    try {
      final response = await _client.get(ApiConstants.revisionRandom);
      return QuizModel.fromJson(response.data as Map<String, dynamic>);
    } on DioException catch (e) {
      throw ApiException.fromDioError(e);
    }
  ...
  Future<Map<String, dynamic>> submitRevisionQuiz(Map<String, List<String>> answers) async {
    try {
      final response = await _client.post(
        ApiConstants.revisionSubmit,
